    ...
    if type(node) == ast.Assert:
        ...
        def get_forest_ass00(cons_pair, dest_pair, node, cons_node):
            ...
            for tt0 in node:
                ...
                ts_res = [list(map((lambda msg_t_ass: (cons_pair((msg_t_ass[0], t_t[1])), cons_node(ts_pred0 + [cons_pair((msg_t_ass[1], t_t[1])),] + ts_succ))), get_forest_ass(t_t[0]))),] + ts_res
        ...
        if type(node) == ast.Sequence:
            ...
            return get_forest_ass0_proc_inl((lambda node0: ast.Proctype(node.name, node0, node.args, node.active, node.d_proc, node.priority, node.provided)) if type(node) == ast.Proctype else (lambda node0: ast.Init(node.name, node0, node.args, node.active, node.d_proc, node.priority, node.provided)))
        elif type(node) == ast.InlineDef and not node.disable_negation:
            return get_forest_ass0_proc_inl(lambda node0: ast.InlineDef(node.name, node.decl, node0))
        else:
            return []
