...
active [0]  proctype send(){
	...
	:: wait_for_receive_blocked_then_suspend_on (target) ->
		...
		event_atomic_surrender_send (current, target, rtems_event_0)
		assert((surrender_send_status == rtems_successful))
		assert((thread_pool[target].event_out_surrender == rtems_event_0))
		event_atomic_surrender_send (current, target, rtems_event_1)
		assert((surrender_send_status == rtems_successful))
		assert((thread_pool[target].event_out_surrender == rtems_event_0))
		suspend_off (target)
	:: send_break ->
...
active [0]  proctype receive(){
	...
	:: else ->
		event_atomic_seize_receive (target, (rtems_event_0 | rtems_event_1), (rtems_event_any | rtems_wait), 1, 3735928559)
		assert((((seize_receive_status == rtems_timeout) && (seize_receive_event_out == 3735928559)) || ((seize_receive_status == rtems_successful) && (seize_receive_event_out == rtems_event_0))))
		event_atomic_seize_receive (target, (rtems_event_0 | rtems_event_1), (rtems_event_any | rtems_no_wait), 0, 3735928559)
		assert((((seize_receive_status == rtems_unsatisfied) && (seize_receive_event_out == 0)) || ((seize_receive_status == rtems_successful) && (seize_receive_event_out == rtems_event_1))))
		retries = (retries - 1)
	od;
	
	printf("//@ 0.8444218515250481 \\<open>1\\<close> \\<open>346\\<close>\n")
}
